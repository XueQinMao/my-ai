...
public class MemoryRecallTool extends AbstractAgentTool {
    ...
    public MemoryRecallTool(AgentRunTracker agentRunTracker,
    ...
    public ToolResult<List<MemoryToolHit>> recallUserMemory(
        ...
        return executeReadonlyTool("recallUserMemory", Map.of("query", query, "limit", limit), "memory", () -> {
            ...
            return memoryRecallService.recall(context.userId(), query, recallLimit).stream().map(
                item -> new MemoryToolHit(item.memoryType(), item.summary(), item.content(), item.importance(),
                    item.confidence())).toList();
        });
    }
}
